...
executor = ThreadPoolExecutor(max_workers=2)   # 用于执行阻塞的 yt-dlp
...
# ====================== 下载核心（使用线程池） ======================
def sync_download(task_id: str, urls: list, quality: str, playlist: bool, cookie_path: str = None):
    """在线程中同步执行 yt-dlp 下载"""
                ...
                if d.get('status') == 'downloading':
                    try:
                        ...
                        asyncio.run_coroutine_threadsafe(
                            send_to_ws(task_id, "progress", percent=percent), loop
                        )
                        asyncio.run_coroutine_threadsafe(
                            send_to_ws(task_id, "log", message=f"{d.get('_percent_str','')} {d.get('_speed_str','')}"), loop
                        )
                    except:
                        pass
...
async def cancel_task(task_id: str):
    await send_to_ws(task_id, "log", message="取消请求已接收")
    return {"success": True}
